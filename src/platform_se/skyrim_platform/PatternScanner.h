...
public:
  ...
  MemoryRegion(){};
  MemoryRegion(uintptr_t b, size_t s) { Set(b, s); }
  uintptr_t base = -1;
  size_t size = -1;
};
...
{
  for (vector<BYTE>::iterator it = pattern.begin(); it != pattern.end();
       *data++, it++) {
    if (*it == mask && useMask)
      continue;
    if (*data != *it)
...
uintptr_t PatternScanInternal(MemoryRegion* r, vector<BYTE> pattern,
                              bool useMask = false, BYTE mask = '\x00')
{
